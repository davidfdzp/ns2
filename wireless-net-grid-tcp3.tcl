# wireless-net-grid-tcp3.tcl: wireless simulation with the following arrangement:
#
#	n0		n1		n2		n3			
#		n4		n5		n6		
#	n7		n8		n9		n10	
...
set opt(finish)         24500                      ;# time to stop simulation (seconds)
...
Application/TcpApp instproc recv {data} {
	global ns num_pkts num_pkts_rx num_pkts_tx minpkttimerx maxpkttimerx maxdelay mindelay last_arrival opt arrival_ app maxstartpkttime minstartpkttime
	set pktrxtime [$ns now]
	$ns trace-annotate "$self received data pkt \"$data\""
	set param [split $data ,]
	set delay [expr $pktrxtime - [lindex $param 0]]
	set srcnode [lindex $param 1]
	set dstnode [lindex $param 2]
	set num_pkts_rx [expr $num_pkts_rx + 1]
	set bpindex [expr $srcnode * $opt(nodes) + $dstnode]
	...
	# Program the sending of the next packet
	set last_arrival($bpindex) $pktrxtime
	...
	if { $startpkttime < [expr 9.0*$opt(finish)/10] } {
		...
		set bpindex [expr $srcnode * $opt(nodes) + $dstnode]
		set bpindex_ [expr $dstnode * $opt(nodes) + $srcnode]
		$ns at $startpkttime "$app($bpindex) send $opt(datasize) {$app($bpindex_) recv {[format {%s,%s,%s} $startpkttime $srcnode $dstnode]}}"
		puts "t = $startpkttime s: node $srcnode will send $opt(datasize) bytes packet to node $dstnode."
...
# Build TCP agents. Each node will try to send 1024 bytes to another node once after $time_btw_pkts_net s on average, if packets are received without interruptions...
for {set i 0} {$i < $opt(nodes)} {incr i} {
	...
	#		Only the first packet sent is programmed, then, when the TCP ACK comes back, 
	# 		a new TCP packet of $opt(datasize) bytes is programmed after $time_btw_pkts_net seconds, and so on...
			set last_arrival($bpindex) 0
			set startpkttime [expr $last_arrival($bpindex) + [$arrival_ value]]
			if {$startpkttime > [expr 9.0*$opt(finish)/10]} {
				puts "Warning: Node $i to node $j packet time $startpkttime exceeds 90 percent of simulation time $opt(finish)."
				if {$startpkttime > $maxfirstpkttime} {
					set maxfirstpkttime $startpkttime
				}
			}
			set last_arrival($bpindex) $startpkttime
			$ns at $startpkttime "$app($bpindex) send $opt(datasize) {$app($bpindex_) recv {[format {%s,%s,%s} $startpkttime $i $j]}}"
