...
# as 140 m < 250 m ok
#
# Each node generates on average a packet per second of 1024 bytes packets towards the others, 
# using ICMP, starting from node 0 to node 1 and so on...
#
...
Agent/Ping instproc recv {from rtt} {
	global ns num_pings_rx num_pings_tx minstartpingtime maxstartpingtime minpingtimerx maxpingtimerx maxfirstpingtime maxrtt minrtt currsrcnode currdstnode last_arrival bp opt
	$self instvar node_
	set pingrxtime [$ns now]
	set currsrcnode [$node_ id]
	set currdstnode $from
	puts "t = $pingrxtime s: node $currsrcnode received ping answer from \
	...
	# Program the next sending of a ping
	set last_arrival(all) $pingrxtime
	set currdstnode [expr $currdstnode + 1]
	# Skip the case when src and dst are the same
	if { $currdstnode == $currsrcnode } {
		set currdstnode [expr $currdstnode + 1]
	}
	...
	if { $currdstnode < $opt(nodes) && $currsrcnode < $opt(nodes) } {
		...
		if { $startpingtime < [expr 9.0*$opt(finish)/10] } {
			...
			puts "t = $startpingtime s: node $currsrcnode will send [$bp($bpindex) set packetSize_] bytes ping request to node $currdstnode."
			...
			puts "Warning: Node $currsrcnode to node $currdstnode ping time $startpingtime exceeds 90 percent of simulation time $opt(finish)."
...
#		Only the first node sends a first ping, then, when the answer comes back, 
# 		a new ping is programmed after $opt(time_btw_pkts) seconds, and so on...
		if { $i == 0 && $j == 1} {
